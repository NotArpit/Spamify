...
% Compute the 'base' error using default values of C = 1, sigma = 0.3 :
model = svmTrain(X, y, C, @(x1, x2) gaussianKernel(x1, x2, sigma)); 
predictions = svmPredict(model, Xval);
error_min = mean(double(predictions ~= yval));

% Nested for loop to consider every combination of [C, sigma] values:
for C_index = 1:values_length,
	for sigma_index = 1:values_length,
